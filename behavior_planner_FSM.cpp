...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = velocity_mag * _lookahead_time + 0.5 * accel_mag * (_lookahead_time * _lookahead_time);  // <- Fix This -> Fixed
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // use cosine and sine to get x and y  <- (Thank U <3)
      //
      auto ang = goal.rotation.yaw + M_PI;
      goal.location.x += _stop_line_buffer*std::cos(ang);  // <- Fix This -> Fixed
      goal.location.y += _stop_line_buffer*std::sin(ang);  // <- Fix This -> Fixed

      // LOG(INFO) << "BP- new STOP goal at: " << goal.location.x << ", "
      //          << goal.location.y;

      // TODO-goal speed at stopping point: What should be the goal speed??
      goal.velocity.x = 0;  // <- Fix This -> Fixed
      goal.velocity.y = 0;  // <- Fix This -> Fixed
      goal.velocity.z = 0;  // <- Fix This -> Fixed
      ...
      goal.velocity.x = _speed_limit * std::cos(goal.rotation.yaw);  // <- Fix This -> Fixed
      goal.velocity.y = _speed_limit * std::sin(goal.rotation.yaw);  // <- Fix This -> Fixed
      goal.velocity.z = 0;
    }
    ...
    if (utils::magnitude(ego_state.velocity) <=
        P_STOP_THRESHOLD_DISTANCE) { // -> Fix this -> Fixed (changed to Distance instead of speed)
      ...
      _active_maneuver = STOPPED;  // <- Fix This -> Fixed
    ...
    // the stop line. goal = ...;
       goal = _goal;  // Keep previous goal. Stay where you are. // <- Fix This -> Fixed
    ...
    if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
      ...
      _active_maneuver = FOLLOW_LANE;  // <- Fix This -> Fixed
